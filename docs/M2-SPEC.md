...
1. **Toon material** (our own `Material`, not `StandardMaterial`):
   ...
   - Color comes from a shared **palette texture** (faces UV-mapped to swatches) plus small detail textures (mortar lines, plank grain, nail heads) generated by our own scripts.
